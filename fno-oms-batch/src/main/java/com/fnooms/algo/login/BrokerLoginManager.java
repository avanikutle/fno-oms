    ...
    public boolean performWebLogin(String brokerPrefix, String totp) {
        try {
            ...
            if (prefix.startsWith("mstock")) {
                ...
                dao.setValue(prefix + ".jwt_token", tokens[0]);
                dao.setValue(prefix + ".refresh_token", tokens[1]);
                log.info("Successfully updated mStock tokens in database for prefix {}.", prefix);
                com.fnooms.util.MStockScripMasterFetcher.fetchAndStoreScripMaster(prefix);
                return true;
                ...
                dao.setValue(prefix + ".jwt_token", tokens[0]);
                dao.setValue(prefix + ".feed_token", tokens[1]);
