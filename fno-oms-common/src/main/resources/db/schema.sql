...
-- ============================================================
CREATE TABLE IF NOT EXISTS angelone_scrip_master (
...
CREATE INDEX IF NOT EXISTS idx_angelone_scrip_master_symbol ON angelone_scrip_master(symbol);

CREATE OR REPLACE TRIGGER trg_angelone_scrip_master_updated_at
    BEFORE UPDATE ON angelone_scrip_master
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- Dhan Scrip Master
-- ============================================================
CREATE TABLE IF NOT EXISTS dhan_scrip_master (
    SEM_EXM_EXCH_ID VARCHAR(20),
    SEM_SEGMENT VARCHAR(20),
    SEM_SMST_SECURITY_ID VARCHAR(50) PRIMARY KEY,
    SEM_INSTRUMENT_NAME VARCHAR(100),
    SEM_TRADING_SYMBOL VARCHAR(100),
    SEM_CUSTOM_SYMBOL VARCHAR(100),
    SEM_EXPIRY_DATE VARCHAR(50),
    SEM_EXPIRY_FLAG VARCHAR(10),
    SEM_STRIKE_PRICE DECIMAL(12, 4),
    SEM_OPTION_TYPE VARCHAR(10),
    SEM_TICK_SIZE DECIMAL(12, 4),
    SEM_LOT_SIZE INT,
    SEM_TRADING_SESSION VARCHAR(50),
    SEM_EXACT_EXPIRY_DATE VARCHAR(50),
    SEM_FREEZE_QUANTITY INT,
    SEM_SERIES VARCHAR(20),
    SEM_INSTRUMENT_TYPE INT,
    SEM_INSTRUMENT_TYPE_NAME VARCHAR(50),
    SEM_ISIN VARCHAR(50),
    updated_by VARCHAR(50) DEFAULT 'SYSTEM',
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
CREATE INDEX IF NOT EXISTS idx_dhan_scrip_symbol ON dhan_scrip_master(SEM_TRADING_SYMBOL);

CREATE OR REPLACE TRIGGER trg_dhan_scrip_master_updated_at
    BEFORE UPDATE ON dhan_scrip_master
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
