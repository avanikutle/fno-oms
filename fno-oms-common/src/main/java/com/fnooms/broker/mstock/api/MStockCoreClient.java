...
/**
 * Core client managing HTTP communication, authentication, and JSON helpers for
 * mStock APIs.
 */
public class MStockCoreClient {
        ...
        try (Response resp = http.newCall(request).execute()) {
            ...
            try {
                ...
                if (root.isJsonArray()) {
                    ...
                    if (arr.size() > 0)
                        json = arr.get(0).getAsJsonObject();
            ...
            if (json.has("status") && !json.get("status").isJsonNull()) {
                ...
                if (!ok) {
                    ...
                    throw new BrokerException("mStock returned status=" + statusEl.getAsString() + ": " + msg,
                            resp.code(), null);
                }
            }
            return json;
    ...
    private Request buildRequest(String url, String jsonBody, String method) {
        ...
        log.info("MSTOCK  - URL: {}", url);
        log.debug("MSTOCK  - Auth Header: {}", authHeader);
        log.debug("MSTOCK  - X-Mirae-Version: {}", apiVersion);
        ...
        switch (method) {
            ...
            case "POST" -> {
                log.info("post jsonbody: {}", jsonBody);
